...
class DicePyTorch(DiceBase):
    ...
    def generate_counterfactuals(self, query_instance, total_CFs, desired_class="opposite", proximity_weight=0.5, diversity_weight=1.0, categorical_penalty=0.1, algorithm="DiverseCF", features_to_vary="all", permitted_option='all', yloss_type="hinge_loss", diversity_loss_type="dpp_style:inverse_dist", feature_weights="inverse_mad", optimizer="pytorch:adam", learning_rate=0.05, min_iter=500, max_iter=5000, project_iter=0, loss_diff_thres=1e-5, loss_converge_maxiter=1, verbose=False, init_near_query_instance=True, tie_random=False, stopping_threshold=0.5, posthoc_sparsity_param=0.1, user_weight = None, alpha = 0.5):
        ...
        :param permitted_option: Either a string "all" or a dictionary containing categorical features with permitted options in respective features.
        ...
        if([total_CFs, algorithm, features_to_vary] != self.cf_init_weights):
            self.do_cf_initializations(total_CFs, algorithm, features_to_vary, permitted_option)
        ...
        self.user_weight = self.process_user_weight(user_weight)
        self.interest_factor = alpha
    ...
    def process_user_weight(self, user_weight):

        user_weight_list = []

        # if user_weight != None:
        for feature in self.data_interface.encoded_feature_names:
            user_weight_list.append(1)
            if user_weight != None:
                for key in user_weight:
                    if key in feature:
                        user_weight_list[-1] = user_weight[key]
                        break
    ...
    def do_cf_initializations(self, total_CFs, algorithm, features_to_vary, permitted_option):
        ...
        if features_to_vary != self.features_to_vary:
            ...
            self.feat_to_vary_idxs = self.data_interface.get_indexes_of_features_to_vary(features_to_vary=features_to_vary, permitted_option = permitted_option)
    ...
    def do_loss_initializations(self, yloss_type, diversity_loss_type, feature_weights):
        ...
        if feature_weights != self.feature_weights_input:
            ...
            if feature_weights == "inverse_mad":
                ...
                # print("Normalized MAD")
                # print(normalized_mads)
                feature_weights = {}
        ...
        # print(self.data_interface.encoded_feature_names)
        # print("Feature weights list")
        # print(feature_weights_list)
    ...
    def compute_dist(self, x_hat, x1):
        ...
        # return torch.sum(torch.mul(torch.mul((torch.abs(x_hat - x1)), self.feature_weights_list), torch.tensor(self.gaze_weights)), dim=0)

    def compute_dist_ele(self, x_hat, x1):
        """Compute weighted distance for each elements between two vectors."""
        return torch.mul((torch.abs(x_hat - x1)), self.feature_weights_list)

    ### For distance computation with user-specific weight
    def compute_dist_weight(self, x_hat, x1):
        # print(self.user_weight)
        # print(torch.mul((torch.abs(x_hat - x1)), self.feature_weights_list))
        return torch.sum(torch.mul(torch.mul((torch.abs(x_hat - x1)), self.feature_weights_list), torch.tensor(self.user_weight)), dim=0)
    ...
    def compute_proximity_loss(self):
        ...
        for i in range(self.total_CFs):
            ...
            proximity_loss += self.compute_dist_weight(self.cfs[i], self.x1)
            ...
            for i in range(len(self.minx[0])):
                if i in self.feat_to_vary_idxs:
                    if init_near_query_instance:
                        ...
                        # else:
                            # self.cfs[n].data[i] = query_instance[i]-((n-2)*0.01)
                    else:
    ...
    def find_counterfactuals(self, query_instance, desired_class, optimizer, learning_rate, min_iter, max_iter, project_iter, loss_diff_thres, loss_converge_maxiter, verbose, init_near_query_instance, tie_random, stopping_threshold, posthoc_sparsity_param):
        ...
        for _ in range(loop_find_CFs):
            ...
            if self.total_random_inits > 0:
                ...
                # self.initialize_CFs(query_instance, True)
            else:
            ...
            while self.stop_loop(iterations, loss_diff) is False:
                ...
                if((self.target_cf_class.item() == 0 and all(i <= self.stopping_threshold for i in test_preds_stored)) | (self.target_cf_class.item() == 1 and all(i >= self.stopping_threshold for i in test_preds_stored))):
            ...
            for j in range(0, self.total_CFs):
                ...
                # print(self.compute_dist_ele(temp, self.x1))
        ...
        if((self.target_cf_class.item() == 0 and any(i[0] > self.stopping_threshold for i in test_preds_stored)) | (self.target_cf_class.item() == 1 and any(i[0] < self.stopping_threshold for i in test_preds_stored))):
